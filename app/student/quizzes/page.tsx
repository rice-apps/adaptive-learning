...
export default function QuizzesPage() {
  ...
  const validateCurrentQuestion = (): boolean => {
    ...
    if (currentQuestion.question_type === "free_response") {
      ...
      if (!answer || !Array.isArray(answer) || answer.length !== details.qa_pairs.length) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // TODO: Save answers to database (you may need to create a QuizAnswers or similar table)
  ...
  const renderReading = (reading?: string) => {
    if (!reading) return null;

    return (
      <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm font-semibold text-blue-900 mb-2">Reading Passage:</p>
        <p className="text-sm text-blue-800 whitespace-pre-wrap">{reading}</p>
      </div>
    );
  };
  ...
  const renderQuestion = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Reading Passage */}
        {renderReading(
          (details as FreeResponseDetails | MCQDetails | DragDropDetails).reading
        )}
                  ...
                  {(details as DragDropDetails).qa_pairs.map((pair, index) => (
                    ...
                    >
                      <div className="flex items-start gap-3 w-full">
                        <span className="text-base font-semibold text-gray-700 mt-0.5">{index + 1}.</span>
                        <p className="text-base text-gray-900 flex-1">{pair.question}</p>
                      </div>
                    </div>
                  ))}
                </div>
