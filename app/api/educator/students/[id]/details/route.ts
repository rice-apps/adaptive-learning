...
export async function GET(
  ...
  try {
    ...
    const weaknessSkills = attributes['weakness-skill'] || attributes['weaknesses-skill'] || [];
    const weaknessDescriptions = attributes['weakness-description'] || attributes['weaknesses-description'] || [];
    ...
    const lessonHistory = assignments.map((assignment: string, index: number) => ({
      assignment,
    ...
    const lastActive = lessonHistory.length > 0 
      ? `${dates[0] || 'Recently'}` 
      : 'No recent activity';
    ...
    return NextResponse.json({
      ...
      lessonHistory: Array.isArray(lessonHistory) ? lessonHistory.slice(0, 20) : [],
