...
// GET /api/educator/quiz-results/[quizId]
// Returns quiz results for an educator to view a student's submitted quiz.
// Verifies the quiz belongs to the educator's student.
export async function GET(
  ...
  try {
    ...
    const { data: quiz, error: quizError } = await serviceSupabase
      .from("Quizzes")
      .select("id, educator_id, student_id, submitted")
      .eq("id", quizId)
      .single();
